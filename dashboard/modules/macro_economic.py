...
# ASEAN Macro Economic Dashboard — World Bank Executive Edition (Static)
# ==========================================================
...
def show():
    ...
    try:
        ...
        st.error("❌ File WDIData.csv tidak ditemukan di folder /data/")
        return
    ...
    df = df[df["Indicator Name"].isin(macro_indicators) & df["Country Name"].isin(asean_countries)]

    df_long = df.melt(id_vars=["Country Name", "Indicator Name"], var_name="Year", value_name="Value")
    df_long = df_long[df_long["Year"].str.isnumeric()]
    df_long["Year"] = df_long["Year"].astype(int)
    df_long["Value"] = pd.to_numeric(df_long["Value"], errors="coerce")
    df_long = df_long[df_long["Year"].between(2009, 2018)]

    # -----------------------------
    # 2️⃣ Pivot Helper
    # -----------------------------
    def pivot_indicator(name):
        sub = df_long[df_long["Indicator Name"] == name]
        pivot = sub.pivot_table(index="Year", columns="Country Name", values="Value")
    ...
    fig.suptitle("ASEAN Macro Economic Dashboard — (2009–2018)",
    ...
    ax1.set_xlabel("Year"); ax1.set_ylabel("%")
    ax1.grid(alpha=0.3)
    ...
    ax3.set_xlabel("Year"); ax3.set_ylabel("% of GDP")
    ax3.grid(alpha=0.3)
    ...
    ax4.set_xlabel("Year"); ax4.set_ylabel("Population (log)")
    ax4.grid(alpha=0.3)
